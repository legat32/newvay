	...
	if (!empty($arResult['SKU_PROPS']))
	{
		foreach ($arResult['SKU_PROPS'] as &$arProp)
		{
			...
			if ('TEXT' == $arProp['SHOW_MODE'])
			{
				...
				if($arProp["CODE"] == "COLOR_LIST") $rail = " scrollbar-rail";
				else $rail = "";
...
'<div class="bx_size_scroller_container'.$rail.'"><div class="bx_size"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list" style="width: '.$strWidth.';">';
				foreach ($arProp['VALUES'] as $arOneValue)
				{
					
					if(isset($arResult["EX_OFFERS_VALUES"][$arOneValue["ID"]]))
					{
					...
					/*
					if(isset($arResult["EX_OFFERS_VALUES_NEW"][$arOneValue["ID"]]))
					{
						$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
						$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-onevalue="'.$arOneValue['ID'].'" style="width: '.$strOneWidth.';" title="'.$arOneValue['NAME'].'"><i></i><span class="cnt">'.$arOneValue['NAME'].'</span></li>';
					}
...
<div class="bx_catalog_list_home col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>"<?if(strLen($arResult["NAV_STRING"])<1):?> style="border-bottom: none;"<?endif?>>
	<?
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	//prn($arItem["PROPERTIES"]["NOVIZNA"]["VALUE"]);
	
	
	?>
	

	<div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?><?if(!is_array($arItem["MIN_PRICE"])):?> retail<?endif?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
	
		
	
		<div class="bx_artikul"><?if(strLen($arItem["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) > 0):?>Арт. <?=$arItem["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]?><?endif?></div>
		
		<div class="bx_catalog_item_title"><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a></div>
	
		<?if($arItem["PROPERTIES"]["AKTSIYA"]["VALUE"] == "true"):?>
			<a class="item_aktsiya_sm" href="<?=$arItem["DETAIL_PAGE_URL"]?>"></a>
		<?endif?>
		
		<?if(($arItem["PROPERTIES"]["AKCIYA_ON_SITE"]["VALUE"] == "Y")&&($arItem["MIN_PRICE"]["DISCOUNT_DIFF"]>0)):?>
			<a class="item_akciya_on_site_sm" href="<?=$arItem["DETAIL_PAGE_URL"]?>"></a>
			<?$arParams['SHOW_DISCOUNT_PERCENT'] = "N";?>
		<?endif?>
	
		<?/*if($arItem["PROPERTIES"]["DAY_THING"]["VALUE_XML_ID"] == "YES"):?>
			<span class="day-thing">
				<span>ВЕЩЬ ДНЯ</span>
				<span class="blink">-15%</span>
			</span>
		<?endif*/?>
		...
		<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="bx_catalog_item_images" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>')" title="<? echo $imgTitle; ?>"><?
			...
			{
			?>
					<div id="<? echo $arItemIDs['DSC_PERC']; ?>" class="bx_stick left bottom" style="display:<? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<? echo $minPrice['DISCOUNT_DIFF_PERCENT']; ?>%</div>
			<?
			}
			if ($arItem['LABEL'])
			{
				//prn($arItem["LABEL_VALUE"]);
			?>
				<div id="<? echo $arItemIDs['STICKER_ID']; ?>" class="bx_stick average left bottom<?if($arItem["LABEL_VALUE"]=="Распродажа"):?> sale<?endif?>" title="<? echo $arItem['LABEL_VALUE']; ?>"><? echo $arItem['LABEL_VALUE']; ?></div>
			<?
			}
			?>
		</a><?
	if ($arItem['SECOND_PICT'])
	{
		...
		{
		?>
			<div id="<? echo $arItemIDs['SECOND_DSC_PERC']; ?>" class="bx_stick left bottom" style="display:<? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<? echo $minPrice['DISCOUNT_DIFF_PERCENT']; ?>%</div>
		<?
		}
		if ($arItem['LABEL'])
		{
		?>
			<div id="<? echo $arItemIDs['SECOND_STICKER_ID']; ?>" class="bx_stick average left bottom<?if($arItem["LABEL_VALUE"]=="Распродажа"):?> sale<?endif?>" title="<? echo $arItem['LABEL_VALUE']; ?>"><? echo $arItem['LABEL_VALUE']; ?></div>
		<?
		}
		?>
		</a><?
	...
	<div class="bx_catalog_item_price">
		<?//prn($arItem["PROPERTIES"]["SMART_SORT"]["VALUE"]);?>
		
		<?
		$price_type_name = "Оптовая цена";
		if(isset($arItem["CATALOG_GROUP_NAME_2"])) $price_type_name = "Для Физ.лиц";
		if(isset($arItem["CATALOG_GROUP_NAME_3"])) $price_type_name = $arItem["CATALOG_GROUP_NAME_3"];
		?>
		
		<div class="bx_price_title"><?=$price_type_name?></div>
		<div  class="bx_price" id="<? echo $arItemIDs['PRICE']; ?>">
			<?if (!empty($minPrice))
			{
...
<?
	...
	else
	{
		...
		<div class="bx_catalog_item_controls no_touch"<?if(!is_array($arItem["MIN_PRICE"])):?> style="display:none;"<?endif?>>
			<?
			if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
			{
			?>
		...
		$boolShowProductProps = false;    // FIX inmark чтобы не отображались лишние свойства
		$boolShowOfferProps = false;	// FIX inmark  чтобы не отображались лишние свойства
...
?>
	...
	<?if((strLen($arResult["DESCRIPTION"]) > 0) && (!isset($_REQUEST["PAGEN_1"]))):?>
