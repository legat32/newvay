...
foreach($arResult["ORDER_PROP"]["USER_PROPS_N"] as $prop_id => &$prop) 
{
	...
	{			
		if($prop["CODE"]=="FIO") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : trim($arUser["NAME"]." ".$arUser["LAST_NAME"]); 
		if($prop["CODE"]=="EMAIL")			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["EMAIL"]; 
		if($prop["CODE"]=="CITY") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["PERSONAL_CITY"];
		if($prop["CODE"]=="PHONE")			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["PERSONAL_PHONE"]; 
		if($prop["CODE"]=="PASSPORT") 		$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_PASSPORT"]; 
		if($prop["CODE"]=="INN_FIZ") 		$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_FIZ_INN"];  
	}
	
	if($arResult["USER_VALS"]["PERSON_TYPE_ID"] == 2) 		// если юрлицо
	{			
		if($prop["CODE"]=="COMPANY") 		$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_COMPANY_NAME"]; 
		if($prop["CODE"]=="COMPANY_ADR") 	$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_COMPANY_ADDRESS"]; 
		if($prop["CODE"]=="INN")			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_INN"]; 		
		if($prop["CODE"]=="KPP") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_KPP"]; 
		if($prop["CODE"]=="OGRN") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_OGRN"]; 
		if($prop["CODE"]=="KOR_SCHET") 		$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_KOR_SCHET"]; 
		if($prop["CODE"]=="R_SCHET") 		$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_R_SCHET"]; 
		if($prop["CODE"]=="CONTACT_PERSON")	$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : trim($arUser["NAME"]." ".$arUser["LAST_NAME"]); 
		if($prop["CODE"]=="EMAIL") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["EMAIL"]; 
		if($prop["CODE"]=="CONTACT_PHONE") 	$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["PERSONAL_PHONE"]; 
		if($prop["CODE"]=="PHONE") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_COMPANY_PHONE"]; 
		if($prop["CODE"]=="FAX") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["UF_FAX"]; 
		if($prop["CODE"]=="CITY") 			$prop["VALUE"] = strLen($prop["VALUE"])>0 ? $prop["VALUE"] : $arUser["PERSONAL_CITY"];
	}
...
function PrintPropsForm($arSource=Array(), $locationTemplate = ".default")
{
	if (!empty($arSource))
	{
		foreach($arSource as $arProperties)
		{
			...
			<tr>
				<td colspan="2"><hr class="razdel" /></td>
			</tr>	
			<?endif?>
...
<p><span class="sof-req">*</span> - <i>поля обязательные для заполнения</i></p>
