...
//prn($arConfig);
if(isset($arConfig["TYPE"])) $type = trim(strVal($arConfig["TYPE"][0]));
else $type = "";
//$interval = 400;
//$type = "csv";
//prn($type);
...
$arColumns = array_merge($arConfig["PRODUCT_FIELDS"], $arConfig["OFFER_FIELDS"]);
array_unshift($arColumns, "TYPE");
$arColumns = array_flip($arColumns);
$arColumns = array_keys($arColumns);
...
while($arRes = $dbRes->GetNext())
{
	...
	if(in_array("DETAIL_PICTURE", $arConfig["PRODUCT_FIELDS"]))
	{
		...
		if($arRes["DETAIL_PICTURE"]>0) 
		{
			...
			if(trim($arConfig["LOGO"][0]) == "N")
			{
				$src = "http://".$_SERVER["SERVER_NAME"].$t["SRC"];
			}
			else
			{
		...
		$tttt = explode("ЦВЕТ", strtoupper($t["DESCRIPTION"]));
		$var = "";
		if(isset($tttt[1]))
			$var = trim(strtoupper(trim($tttt[1])));
		else 
			$var = trim(strtoupper(trim($tttt[0])));
		$arGoods[$arRes["ID"]]["DETAIL_PICTURE"] = "[".$var."|".$src."]";
	}
	...
	if(in_array("MORE_PHOTO", $arConfig["PRODUCT_FIELDS"]))
	{
		if($arRes["PROPERTY_MORE_PHOTO_VALUE"]>0)
		{
			...
			$arGoods[$arRes["ID"]]["MORE_PHOTO"] .= "[".$t["DESCRIPTION"]."|".$src."]";
		}
	}
	$num++;
	//if($num>1000) break;
...
if($type == "csv")
{
	header('Content-Type: text/csv; charset=win-1251'); 
	header('Content-Disposition: attachment; filename="report.csv"'); 
	echo implode(";", $arColumns)."\n";
	echo "STAT;".$step."-".($step+$interval)." from ".count($arID)."\n";
	foreach($arGoods as $arGood)
	{
		...
		foreach($arColumns as $field) 
		{
			...
			echo iconv( "utf-8", "windows-1251", $arGood[$field]).";";
		}
		echo "\n";
		...
		foreach($arGood["OFFERS"] as $arOffer)
		{
			...
			{
				if($field == "TYPE") continue;
				echo iconv( "utf-8", "windows-1251", $arOffer[$field]).";";
			}
			echo "\n";
		}
	}
