$(document).ready(function() {
	...
	//$(".bx_filter_container").eq(0).find(".bx_filter_block span").eq(1).find("input").attr("checked", "checked");
	//$(".bx_filter_container").eq(0).find(".bx_filter_block span").addClass("disabled").find("input").attr("disabled", "disabled");
	$(".bx_filter_container").eq(0).hide();
	$(".bx_filter_container").eq(2).after("<div style='clear:both;'></div>");
	
	$(".bx_filter_container").each(function(index) {
		if(index>2) {
			$(this).addClass("collapsed");
			$(this).find(".bx_filter_block").find("span").each(function( index ) {
				if(!$(this).find("input[type=checkbox]").is(":checked")) $(this).hide();
				});
			$(this).find(".bx_filter_container_title").append("<span class='to_show'></show>");
			}
		});

	// bind collapse/expand event
	$(".bx_filter_container_title").on("click", function() {
		if($(this).parent("div").index()>3) {
			if($(this).parent("div").hasClass("collapsed")) {
				$(this).parent("div").removeClass("collapsed");
				$(this).parent("div").find(".bx_filter_block span").slideDown("fast");
				$(this).find("span").remove();
				$(this).append("<span class='to_hide'></show>");
				}
			else {
				$(this).parent("div").addClass("collapsed");
				$(this).parent("div").find(".bx_filter_block").find("span").each(function( index ) {
					if(!$(this).find("input[type=checkbox]").is(":checked")) $(this).slideUp("fast");
					});
				$(this).find("span").remove();
				$(this).append("<span class='to_show'></show>");
				}
			}
		});
