...
if(!$arResult["OFFERS"][0]["MORE_PHOTO_COUNT"]) $arParams["DETAIL_PICTURE_MODE"] = "";
...
//$arRes = setElementMorePhoto($arResult, $WIDTH = 1000, $HEIGHT = 1500);
...
//prn($arResult['MORE_PHOTO']);
?>

	<?if($USER->isAdmin()):?>
		<a style="padding:5px 10px; background:green; color:white; display:inline-block; margin-bottom:10px;" href="/tools/img_sync.php?xml=<?=$arResult["XML_ID"]?>" target="_blank">Обновить картинки с NEWVAY.RU</a>
	<?endif?>
	...
	<div class="bx_item_container">
		...
		<div class="bx_lt">
			...
			?>
				<li data-value="<? echo $arOneOffer['ID'].'_'.$arOnePhoto['ID']; ?>" style="width: <? echo $strOneWidth; ?>; padding-top: <? echo $strOneWidth; ?>">
					<?//prn($arOnePhoto)?>
					<span class="cnt"><span class="cnt_item" style="background-image:url('<? echo $arOnePhoto['SRC']; ?>');"></span></span>
				</li>
			<?
						}
						unset($arOnePhoto);
		...
		<div class="bx_rt">
			...
			?>
			<div class="item_price"<?=!$boolDiscountShow ? ' style="height: 70px;"' : ''?>>
			...
			<div class="offers">
				...
				?>	
					...
					<div class="item_buttons vam">
						<span class="item_buttons_counter_block cnt">
				...
				?>
					...
					<?if ($arParams['SHOW_BASIS_PRICE'] == 'Y')
					{
						...
						?>
						<p id="<? echo $arItemIDs['BASIS_PRICE']; ?>" class="item_basis_price"><? echo GetMessage('CT_BCE_CATALOG_MESS_BASIS_PRICE', $basisPriceInfo); ?></p>
			...
			<?/*?>
			<div class="item_info_section offers">
			...
			<div class="artikul-block">
				<span class="artikul-name">Артикул:</span>
				<span class="artikul-value"><?=$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]?></span>
			</div>
			...
			if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']) && !empty($arResult['OFFERS_PROP']))
			{
				$arSkuProps = array();
			?>
			<div class="item_info_section scu" id="<? echo $arItemIDs['PROP_DIV']; ?>">
			
			
			<?
			// подсчитаем вручную чтобы исключить пустые значения
			$values_count = 0;
			foreach($arResult["SKU_PROPS"]["COLOR"]["VALUES"] as $val)
				if($val["ID"] > 0) $values_count++;
			?>
			...
			?>
				<div class="<? echo $strClass; ?>" id="<? echo $arItemIDs['PROP'].$arProp['ID']; ?>_cont">
					...
					<?if($arProp["CODE"] == "SIZE"):?>
						<a class="get_size fancybox-iframe" href="/razmery/razmery_popup.php">Определить размер</a>
						<a class="get_size_min" href="/razmery/">Определить размер</a>
					<?endif?>
					
					<div class="bx_size_scroller_container<?if($arProp["CODE"] == "COLOR"):?> scrollbar-rail<?endif?>">
						<div class="bx_size">
			...
			<?//pra($arResult["DISPLAY_PROPERTIES"]);?>
			...
			if ('' != $arResult['PREVIEW_TEXT'])
			{
				...
				{
					$about .= ('html' == $arResult['PREVIEW_TEXT_TYPE'] ? $arResult['PREVIEW_TEXT'] : '<p><b>'.$arResult['PREVIEW_TEXT'].'</b></p>');
				}
			}
			...
			if ('' != $arResult['DETAIL_TEXT'])
			{
				if ('html' == $arResult['DETAIL_TEXT_TYPE']) 
					...
					$about .= "<p>".$arResult['DETAIL_TEXT']."</p>";
			}
			*/
			
			/*
			$isOne = false;  // флаг что есть хотя бы одно заполненное свойство
			if(is_array($arResult["DISPLAY_PROPERTIES"]) && (count($arResult["DISPLAY_PROPERTIES"]) > 0))
			{
				$about .= "<dl class='good_props'>";
				foreach($arResult["DISPLAY_PROPERTIES"] as $arProp)
				{
					if(strLen($arProp["DISPLAY_VALUE"])>0)
					{
						$about .= "<dt>".$arProp["NAME"]."</dt>";
						$about .= "<dd>".$arProp["DISPLAY_VALUE"]."</dd>";
						$isOne = true;
					}
				}
				$about .= "</dl>";
			...
			if(!$isOne) $about = "";
			*/
			$about = $arResult["PROPS_TABLE"];
			
			
			$f = @file($_SERVER["DOCUMENT_ROOT"]."/include/product_dostavka.php");
			$dostavka = implode("", $f);
			
			$f = @file($_SERVER["DOCUMENT_ROOT"]."/include/product_oplata.php");
			$oplata = implode("", $f);
			...
			if(!empty($arResult["PREVIEW_PICTURE"]["SRC"])) 
				$pic = ' data-image="http://'.$_SERVER["SERVER_NAME"].$arResult["PREVIEW_PICTURE"]["SRC"].'"';
			...
			<?$APPLICATION->IncludeComponent(
				...
				array(
					...
					"DATA_IMAGE" => $arResult["PREVIEW_PICTURE"]["SRC"],
					"DATA_SERVICES" => array(
						0 => "vkontakte",
						1 => "facebook",
						2 => "odnoklassniki",
					...
					"DATA_TYPE" => "counters",
					"DATA_URL" => substr($_SERVER["REQUEST_URI"],0,strpos($_SERVER["REQUEST_URI"],"?")),
					"INMARK_COUNT_ELEMENTS" => "4",
					"INTRO_TEXT" => ""
				),
				false
			);?>
