...
//pra($arResult["ITEMS"][0]);
//pra($arResult["ITEMS"][0]["PREVIEW_PICTURE"]);
//pra($arResult["ITEMS"][0]["DETAIL_PICTURE"]);
...
//pra($arResult["ITEMS"][0]["PREVIEW_PICTURE"]);
	...
	foreach($arResult["ITEMS"][5]["OFFERS"] as $arOffer)
	{
		//pra($arOffer["NAME"]);
		//pra($arOffer["PREVIEW_PICTURE"]);
		//pra($arOffer["DETAIL_PICTURE"]);
	}
	foreach($arResult["ITEMS"][6]["OFFERS"] as $arOffer)
	{
		//pra($arOffer["NAME"]);
		//pra($arOffer["PREVIEW_PICTURE"]);
		//pra($arOffer["DETAIL_PICTURE"]);
...
if (!empty($arResult['ITEMS']))
{
	...
	$strEmptyPreview = '/upload/no_female.png';
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
			{
				...
				// закомментили, так как вверху своя сортировка offers
	...
	// ======================================================================================
	// Исправим адреса карточек товаров / уберем стикер "Основной каталог"
	// ======================================================================================
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		//{
			...
			if($arItem["PROPERTIES"]["NOVIZNA"]["VALUE"] == "Основной каталог")
			{
				unset($arResult['ITEMS'][$key]["LABEL"]);
			}
		//}
	}
	...
	{
		foreach ($arItem['OFFERS'] as $kOffer => $arOffer)
		{
			//$arResult["ITEMS"][$kItem]["OFFERS"][$kOffer]["PREVIEW_PICTURE"] = $arOffer["DETAIL_PICTURE"];
			//pra($arOffer["PREVIEW_PICTURE"]);
			//pra($arOffer["DETAIL_PICTURE"]);
		}
	}
	...
	// получим перечень значений свойств торговых предложений, дабы избежать вывода всех возможных вариантов на страницу в коде (баг! не баг, не знаю)
	$arExOffersValues = Array();
	foreach($arResult["ITEMS"] as $kItem => $arItem)
		foreach($arItem["OFFERS"] as $arOffer)
			foreach($arOffer["TREE"] as $prop_code => $prop_value)
			{
				$arExOffersValues[$prop_value] = 1;
				$arExOffersValuesNew[$arItem["ID"]][$prop_value] = 1;
			}
	$arResult["EX_OFFERS_VALUES"] = $arExOffersValues;
	$arResult["EX_OFFERS_VALUES_NEW"] = $arExOffersValuesNew;   // Попытка еще уменьшить количество кода на странице
	//prn($arExOffersValues);
	//prn($arExOffersValuesNew);
	
	
	
	
	// подменим названия для вывода названий свойств
	$arResult['SKU_PROPS']["COLOR_LIST"]["NAME"] = "Цвет";
	$arResult['SKU_PROPS']["SIZE_LIST"]["NAME"] = "Размер";
