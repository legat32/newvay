...
<?if(($arResult["VALUES"]["LOGIN"] == $arResult["VALUES"]["EMAIL"]) && (!empty($arResult["VALUES"]["LOGIN"])) &&(count($arResult["ERRORS"])<1) ):?>
	<p><?ShowMessage(Array("TYPE"=>"OK", "MESSAGE"=>GetMessage("REGISTER_EMAIL_WAS_SENT")));?></p>
<?endif?>

<p>Если Вы уже регистрировались на сайте и забыли пароль воспользуйтесь ссылкой <a href="/personal/?forgot_password=yes">Восстановить пароль</a></p>
	...
	<?if( empty($arResult["bVarsFromForm"]) || count($arResult["ERRORS"]) > 0 ):?>
		...
		<form method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data">
			...
			<?foreach ($arResult["SHOW_FIELDS"] as $FIELD):?>
				...
				{
					case "PASSWORD":
						?><input size="30" type="password" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" class="bx-auth-input" /> не менее 6 символов
