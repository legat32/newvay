...
$count = $APPLICATION->get_cookie('count') ? $APPLICATION->get_cookie("count") : "30"; 

if(($arResult["VARIABLES"]["SECTION_ID"] == 303) && (!isset($_GET["sort"])))
{
...
if(isset($_REQUEST["count"])) {
	$APPLICATION->set_cookie("count", strVal($_REQUEST["count"]), 30); 
...
if (CModule::IncludeModule("iblock") && COption::GetOptionString("eshop", "catalogSmartFilter", "Y", SITE_ID)=="Y")
{
	...
	<?/*if($USER->isAdmin()):?>
		...
		{
			$arSects = Array();
			$dbRes = CIBlockSection::GetList(Array(), Array("ACTIVE" => "Y", "IBLOCK_ID" => 6, "SECTION_ID" => $arCurSection, "CNT_ACTIVE" => "Y"), true);
			while($arRes = $dbRes->GetNext())
			{
				//prn($arRes["NAME"]." (".$arRes["DEPTH_LEVEL"].") ".$arRes["ELEMENT_CNT"]);
				if($arRes["ELEMENT_CNT"]>0) $arSects[] = $arRes;
			}
			//foreach($arSects as $arSect) prn($arSect["NAME"]." (".$arSect["DEPTH_LEVEL"].") ".$arSect["ELEMENT_CNT"]);
		}
		?>
		<?if(count($arSects) > 0):?>
			<div id="section-list-top">
				<ul>
...
//pra($element_sort_field);
//pra($element_sort_order);
//pra($arParams["FILTER_NAME"]);
//pra($GLOBALS["arrFilter"]);
...
if($USER->isAdmin())
{
	...
	$dbRes = CIBlockElement::GetList(Array(), Array("IBLOCK_ID" => 6, "SECTION_ID" => 280, "ACTIVE" => "Y"), false, false, Array("ID", "NAME"));
	$num = 0;
	while($arRes = $dbRes->GetNext()) 
	{
		$num++;
		echo $num.". ".$arRes["NAME"]."<br>";	
	}
}
*/
...
<?$APPLICATION->IncludeComponent(
	...
	Array(
		...
		"PAGE_ELEMENT_COUNT" => $count == 30 ? 30 : 60,
