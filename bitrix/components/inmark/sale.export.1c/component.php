		...
		if($ABS_FILE_NAME)
		{
			...
			if(strlen($_SESSION["BX_CML2_EXPORT"]["zip"])>0)
			{
				...
				if(class_exists("ZipArchive"))
				{
					...
					{
						$zip = new ZipArchive;
						$zipOpened = $zip->open($file_name, ZIPARCHIVE::CHECKCONS);
					...
					if($zipOpened === true)
					{
						for ($i = 0; $i < $zip->numFiles; $i++)
						{
							...
							//Check for directory
							if($entry_stat['size'] > 0 && strpos($entry_name,".xml") !== false)
							{
								$fin = $zip->getStream($entry_name);
								if ($fin)
								{
...
*/
					...
					function nodeHandler(CDataXML $value)
					{
						...
						if($value["#"][GetMessage("CC_BSC1_OPERATION")][0]["#"] == GetMessage("CC_BSC1_ORDER"))
						{
							...
							$orderId = $value["#"][GetMessage("CC_BSC1_NUMBER")][0]["#"];

							if($accountNumberPrefix == "" || strpos($orderId, $accountNumberPrefix) === 0)
							{
								if($accountNumberPrefix != "")
									$orderId = substr($orderId, strlen($accountNumberPrefix));
								...
								$GLOBALS["arOrders"][$orderId][] = $v;
								//die();

								$dbOrder = CSaleOrder::GetList(array(), array("ACCOUNT_NUMBER" => $orderId));
								...
								if($orderInfo = $dbOrder->Fetch())
								{
									...
									if($orderInfo["PAYED"] != "Y" && $orderInfo["ALLOW_DELIVERY"] != "Y" && $orderInfo["STATUS_ID"] != "F")
									{
										...
										while ($arBasket = $dbBasket->Fetch())
										{
											...
											{
												if($v["TRAITS"][GetMessage("CC_BSC1_CANCELED")] != "true" && $orderInfo["CANCELED"] == "N")
										...
										$arOrderFields = Array();
										if(DoubleVal($v["AMOUNT"]) > 0 && $v["AMOUNT"] != $orderInfo["PRICE"])
				...
				*/
				...
				foreach($GLOBALS["arOrders"] as $orderId => $arOrder) 
				{
					...
					if(count($arOrder)>1) 
					{
						...
						$arNewOrder = $arOrder[0];  
						
						for($i=1; $i<count($arOrder); $i++) 
						{
							$arNewOrder["AMOUNT"] += $arOrder[$i]["AMOUNT"];
							foreach($arOrder[$i]["items"] as $item_code => $item) 
							{
								if(isset($arNewOrder["items"][$item_code])) $arNewOrder["items"][$item_code]["QUANTITY"] += $item["QUANTITY"];
								else $arNewOrder["items"][$item_code] = $item;
							}
						}
