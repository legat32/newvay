...
$form_type = strVal($_GET["country"])."_".strVal($_GET["type"]);
if($form_type == "ru_ooo")
{
	$arResult["SHOW_FIELDS"] 		= Array("EMAIL", "PASSWORD", "CONFIRM_PASSWORD", "UF_COMPANY_NAME", "UF_COMPANY_ADDRESS", "UF_INN", "UF_KPP", "UF_OGRN", "UF_CITY", "UF_OKRUG", "UF_ASSORTIMENT", "UF_BIZNES_TYPE", "NAME", "SECOND_NAME", "LAST_NAME", "PERSONAL_PHONE", "PERSONAL_NOTES");
	$arResult["REQUIRED_FIELDS"] 	= Array("EMAIL", "PASSWORD", "CONFIRM_PASSWORD", "UF_COMPANY_NAME", "UF_COMPANY_ADDRESS", "UF_INN", "UF_OGRN", "UF_CITY", "UF_OKRUG", "UF_ASSORTIMENT");
}
...
?>
	...
	<?
	function showFields($arResult, $arFields) 
	{
		foreach ($arResult["SHOW_FIELDS"] as $FIELD):?>
			<?if(!in_array($FIELD, $arFields)) continue;?>
	...
	<?
	function showUserFields($arResult, $arRequiredUserFields, $arFields)
	{
		global $APPLICATION;
		
		foreach ($arResult["USER_PROPERTIES"]["DATA"] as $FIELD_NAME => $arUserField):?>
			<?if(!in_array($FIELD_NAME, $arFields)) continue;?>
...
<?else:?>
	...
	<?if( empty($arResult["bVarsFromForm"]) || count($arResult["ERRORS"]) > 0 ):?>
		...
		<form method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data">
			...
			<?showFields($arResult, Array("EMAIL", "PASSWORD", "CONFIRM_PASSWORD"));?>
			<?showUserFields($arResult, $arRequiredUserFields, Array("UF_COMPANY_NAME", "UF_COMPANY_ADDRESS", "UF_INN", "UF_KPP", "UF_OGRN", "UF_CITY", "UF_OKRUG", "UF_ASSORTIMENT", "UF_BIZNES_TYPE"));?>
			<?showFields($arResult, Array("NAME", "SECOND_NAME", "LAST_NAME", "PERSONAL_PHONE", "PERSONAL_NOTES"));?>
			<?
			/*
			$arPart = Array();
			$last_uf = false;
			...
			{
				//prn($FIELD);
				if(substr($FIELD, 0, 3) == "UF_") $uf = true; else $uf = false;
				if($uf == $last_uf) $arPart[] = $FIELD;
				if($uf != $last_uf) 
				{
					//prn($arPart);
					if($last_uf) showUserFields($arResult, $arRequiredUserFields, $arPart);
					else showFields($arResult, $arPart);
					$arPart = Array($FIELD);
					$last_uf = $uf;
				}
				//prn($FIELD);
