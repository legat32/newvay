...
if( $obCache->InitCache($cacheLifetime, $cacheID, $cachePath) ) {
	$arDetailPageURL = $obCache->GetVars();
	//echo "from cache<hr/>"; 
	}
elseif( $obCache->StartDataCache()  ) {
	$arDetailPageURL = Array();
	$dbRes = CIBlockElement::GetList(Array(), Array("IBLOCK_ID" => 6, "ACTIVE" =>"Y"), false, false, Array("ID", "DETAIL_PAGE_URL"));
	while($arRes = $dbRes->GetNext()) $arDetailPageURL[$arRes["ID"]] = $arRes["DETAIL_PAGE_URL"];
	$obCache->EndDataCache($arDetailPageURL);
