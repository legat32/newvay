...
<?foreach($arResult["SECTIONS"] as $arSection):?>
	<?if($arSection[DEPTH_LEVEL]==1):?>
		 ...
		 <section class="<?=$arSection[IPROPERTY_VALUES][SECTION_PAGE_TITLE]?>">
		 		<?if(!empty($arSection[DETAIL_PICTURE])):?>
					<?$img = CFile::GetFileArray($arSection[DETAIL_PICTURE]);?>
					
					<div class="section-image"><a href="<?=$arSection[SECTION_PAGE_URL]?>"><img width="<?=$img[WIDTH]?>" height="<?=$img[HEIGHT]?>" src="<?=$img[SRC]?>"/></a>
                ...
                <div class="section-content section-<?=$arSection[ID];?>">
                    ...
                    <div class="count">ТОВАРОВ:  <span><?=$arSection[ELEMENT_CNT]?></span></div>
                	</a>
