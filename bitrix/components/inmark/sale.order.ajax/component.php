...
if (!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "N")
{
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
	{
		...
		elseif ($_POST["do_register"] == "Y" && $arResult["AUTH"]["new_user_registration"] == "Y")
		{
			...
			if (strlen($_POST["NEW_CITY"]) <= 0)
				$arResult["ERROR"][] = GetMessage("STOF_ERROR_REG_CITY");
			
			if (strlen($_POST["NEW_PHONE"]) <= 0)
				$arResult["ERROR"][] = GetMessage("STOF_ERROR_REG_PHONE");
			...
			$arResult["AUTH"]["~NEW_CITY"] = $_POST["NEW_CITY"];
			$arResult["AUTH"]["NEW_CITY"] = htmlspecialcharsEx($arResult["AUTH"]["~NEW_CITY"]);
			$arResult["AUTH"]["~NEW_PHONE"] = $_POST["NEW_PHONE"];
			$arResult["AUTH"]["NEW_PHONE"] = htmlspecialcharsEx($arResult["AUTH"]["~NEW_PHONE"]);
		}
	}
}
...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		while ($arItem = $dbBasketItems->GetNext())
		{
			...
			$totalCountGoods += $arItem["QUANTITY"];
		...
		if(($totalCountGoods < 10)&&( (defined("DEALER_USER")) || (defined("JOINT_USER")) )) 
		{
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
					...
					$dbBasketItems = CSaleBasket::GetList(
						...
						array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "PRICE",
							"WEIGHT", "NAME", "DISCOUNT_PRICE", "CURRENCY", "PRODUCT_PROVIDER_CLASS", "DIMENSIONS", "TYPE", "SET_PARENT_ID"
						)
					);
					while ($arItem = $dbBasketItems->GetNext())
						$arOrderForDiscount['BASKET_ITEMS'][$arItem["ID"]] = $arItem;

					// set weight for the set parent
					foreach ($arOrderForDiscount['BASKET_ITEMS'] as $arItem)
					{
						if (CSaleBasketHelper::isSetItem($arItem))
							$arOrderForDiscount['BASKET_ITEMS'][$arItem["SET_PARENT_ID"]]['WEIGHT'] += $arItem['WEIGHT'] * $arItem['QUANTITY'];
					}
					...
					{
						if (!CSaleBasketHelper::isSetItem($arItem))
							$arOrderForDiscount['ORDER_WEIGHT'] += doubleval($arItem['WEIGHT']);
					}
				...
				//$dbBasketItems = CSaleBasket::GetList( Array(), Array( "LID" => SITE_ID, "ORDER_ID" => $arResult["ORDER_ID"])/*, false, false, Array("PRODUCT_ID") */);
				/*
				while ($arItem = $dbBasketItems->GetNext()) {
					$arOffersIds[] = $arItem["PRODUCT_ID"];
					prn($arItem);
					}
				die();
				*/
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if (intval($arUserResult["PAY_SYSTEM_ID"]) != 0)
					{
						$arFilter["RELATED"]["PAYSYSTEM_ID"] = $arUserResult["PAY_SYSTEM_ID"];
						$arFilter["RELATED"]["TYPE"] = "WITH_NOT_RELATED";
					}

					if (strlen($arUserResult["DELIVERY_ID"]) > 0)
					{
						$arFilter["RELATED"]["DELIVERY_ID"] = $arUserResult["DELIVERY_ID"];
						$arFilter["RELATED"]["TYPE"] = "WITH_NOT_RELATED";
					}
					...
					while ($arOrderProperties = $dbOrderProperties->Fetch())
					{
						...
						if ($arOrderProperties["TYPE"] == "FILE" && is_array($curVal))
						{
							$tmpVal = "";
							foreach ($curVal as $index => $arFile)
							{
								if (isset($arFile["name"]) && strlen($arFile["name"]) > 0)
								{
				...
				$arOffersIds = Array();
				$dbBasketItems = CSaleBasket::GetList( Array(), Array( "LID" => SITE_ID, "ORDER_ID" => $ORDER_ID), false, false, Array("PRODUCT_ID") );
				while ($arItem = $dbBasketItems->GetNext()) {
					$arOffersIds[] = $arItem["PRODUCT_ID"];
					//prn($arItem);
					}
				...
				$arOrders = Array();
				$dbRes = CIBlockElement::GetList(Array(), Array("IBLOCK_ID"=>$OFFERS_IBLOCK, "ACTIVE"=>"Y", "ID" => $arOffersIds), false, false, Array("ID", "NAME", "PROPERTY_CML2_LINK.PROPERTY_COLLECTION"));
				while($arRes = $dbRes->GetNext()) {
				 //$arFields = $ob->GetFields();
				 //print_r($arFields);
					if(in_array($arRes["PROPERTY_CML2_LINK_PROPERTY_COLLECTION_VALUE"], Array("VAY", "JW")) ) $arOrders["WOMEN"][] = $arRes["ID"];
					//if(in_array($arRes["PROPERTY_CML2_LINK_PROPERTY_COLLECTION_VALUE"], Array("VESNUSHKI", "VAY_KIDS")) ) $arOrders["CHILDREN"][] = $arRes["ID"];
					if(in_array($arRes["PROPERTY_CML2_LINK_PROPERTY_COLLECTION_VALUE"], Array("DETSKIYTRIKOTAZH", "VESNUSHKI", "VAYKIDS")) ) $arOrders["CHILDREN"][] = $arRes["ID"];
					//if(in_array($arRes["PROPERTY_CML2_LINK_PROPERTY_COLLECTION_VALUE"], Array("DETSKIYTRIKOTAZH")) ) $arOrders["CHILDREN"][] = $arRes["ID"];
					}
				//prn($arOrders);
				//die();
				...
				if(count($arOrders) >1) 
				{
					...
					$ORDER_WOMEN = $ORDER_ID;
					$ORDER_CHILDREN = false;
					
					$arOrderWomen = CSaleOrder::GetByID($ORDER_WOMEN);
					
					
					// создадим новый заказ для детского ассортимента

					$arFields = array(
						"LID" => SITE_ID,
						"PERSON_TYPE_ID" => $arOrderWomen["PERSON_TYPE_ID"],
						"PAYED" => "N",
						"CANCELED" => "N",
						"STATUS_ID" => "N",
						"PRICE" => 0,
						"CURRENCY" => $arOrderWomen["CURRENCY"],
						"USER_ID" => $arOrderWomen["USER_ID"],
						"PAY_SYSTEM_ID" => $arOrderWomen["PAY_SYSTEM_ID"],
						"PRICE_DELIVERY" => $arOrderWomen["PRICE_DELIVERY"],
						"DELIVERY_ID" => $arOrderWomen["DELIVERY_ID"],
						"DISCOUNT_VALUE" => $arOrderWomen["DISCOUNT_PRICE"],
						"TAX_VALUE" => $arOrderWomen["TAX_VALUE"],
						"USER_DESCRIPTION" => $arOrderWomen["USER_DESCRIPTION"]
					);

					$ORDER_CHILDREN = CSaleOrder::Add($arFields);
					$ORDER_CHILDREN = IntVal($ORDER_CHILDREN);
					if ($ORDER_CHILDREN <= 0)
					{
						if($ex = $APPLICATION->GetException())
							$arResult["ERROR"][] = $ex->GetString();
					...
					{
						$arOrderChildren = CSaleOrder::GetByID($ORDER_CHILDREN);
					}
					//prn($arOrderChildren);
					
					
					// переместим детские заказы из исходного в новый заказ
					
					$dbBasketItems = CSaleBasket::GetList(
						array("NAME" => "ASC"), 
						array(
								"LID" => SITE_ID,
								"ORDER_ID" => $ORDER_WOMEN
							),
						false,
						false,
						array()
					...
					while ($arItem = $dbBasketItems->GetNext())
					{
						if(in_array($arItem["PRODUCT_ID"], $arOrders["CHILDREN"])) 
						{
							//echo $arItem["PRODUCT_ID"]." => CHILDREN<br/>";
							$arFields = array( "ORDER_ID" => $ORDER_CHILDREN );
							//prn($arFields);
					...
					// теперь еще надо пересчитать итоговые суммы обоих заказов
					...
					while ($arItem = $dbBasketItems->GetNext())
					{
						$orderWomenSum += $arItem["PRICE"] * $arItem["QUANTITY"];
					}
					//echo "<hr/>".$orderWomenSum."<hr/>";
					...
					while ($arItem = $dbBasketItems->GetNext())
					{
						$orderChildrenSum += $arItem["PRICE"] * $arItem["QUANTITY"];
					}
					//echo "<hr/>".$orderChildrenSum."<hr/>";
					
					
					CSaleOrder::Update($ORDER_CHILDREN, array("PRICE" => $orderChildrenSum));
					CSaleOrder::Update($ORDER_WOMEN, array("PRICE" => $orderWomenSum));
					
					
					
					
					// и еще надо скопировать свойства заказа из женского в детский
					
					$db_vals = CSaleOrderPropsValue::GetList( array("SORT" => "ASC"), array("ORDER_ID" => $ORDER_WOMEN) );
					while($arVals = $db_vals->Fetch()) {
						$arFields = $arVals;
						unset($arFields["ID"]);
						$arFields["ORDER_ID"] = $ORDER_CHILDREN;
						CSaleOrderPropsValue::Add($arFields);
						}	
				...
				// выясняем и вносим в заказ специфические свойства
				
				//$arFields = array( "ORDER_ID" => $ORDER_ID, "CODE" => "ORDER_CATEGORY", "VALUE" => "чистый шелк" );
				//CSaleOrderPropsValue::Add($arFields);
				
				
				
				// общие свойства заказов
				
				$arPrice = CCatalogGroup::GetByID(PRICE_TYPE);
				$price_type = $arPrice["NAME"];
				
				$basket_discount = defined("DEALER_USER") ? DEALER_USER : 0;
				//echo $basket_discount;
				//die();
				...
				if(count($arOrders) == 1) 				
				{
					$k = array_keys($arOrders);
					$group_kod = $k[0];
					$group_name = ($group_kod == "CHILDREN") ? "Детский ассортимент" : "Женский ассортимент";
					
					if($arOrder["PERSON_TYPE_ID"]==1) // если физлицо
					{
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 26, "NAME" => "Группа товаров в заказе", "CODE" => "ORDER_CATEGORY", "VALUE" => $group_name);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 27, "NAME" => "Типы цен в заказе", "CODE" => "PRICE_TYPE", "VALUE" => $price_type);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 30, "NAME" => "Скидка к корзине", "CODE" => "BASKET_DISCOUNT", "VALUE" => $basket_discount);
						CSaleOrderPropsValue::Add($arFields);
					}
					
					if($arOrder["PERSON_TYPE_ID"]==2) // если юрлицо
					{
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 28, "NAME" => "Группа товаров в заказе", "CODE" => "ORDER_CATEGORY", "VALUE" => $group_name);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 29, "NAME" => "Типы цен в заказе", "CODE" => "PRICE_TYPE", "VALUE" => $price_type);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 31, "NAME" => "Скидка к корзине", "CODE" => "BASKET_DISCOUNT", "VALUE" => $basket_discount);
						CSaleOrderPropsValue::Add($arFields);
					}
				...
				// ================= если заказ разделился на два
				
				if(count($arOrders)>1) 
				{
				
					// обработка женского заказа
					
					$ORDER_ID = $ORDER_WOMEN;
					$group_kod = "WOMEN";
					$group_name = "Женский ассортимент";
					
					if($arOrderWomen["PERSON_TYPE_ID"]==1) // если физлицо
					{
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 26, "NAME" => "Группа товаров в заказе", "CODE" => "ORDER_CATEGORY", "VALUE" => $group_name);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 27, "NAME" => "Типы цен в заказе", "CODE" => "PRICE_TYPE", "VALUE" => $price_type);
					...
					if($arOrderWomen["PERSON_TYPE_ID"]==2) // если юрлицо
					{
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 28, "NAME" => "Группа товаров в заказе", "CODE" => "ORDER_CATEGORY", "VALUE" => $group_name);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 29, "NAME" => "Типы цен в заказе", "CODE" => "PRICE_TYPE", "VALUE" => $price_type);
					...
					// обработка детского заказа
					
					$ORDER_ID = $ORDER_CHILDREN;
					$group_kod = "CHILDREN";
					$group_name = "Детский ассортимент";
					
					if($arOrderChildren["PERSON_TYPE_ID"]==1) // если физлицо
					{
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 26, "NAME" => "Группа товаров в заказе", "CODE" => "ORDER_CATEGORY", "VALUE" => $group_name);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 27, "NAME" => "Типы цен в заказе", "CODE" => "PRICE_TYPE", "VALUE" => $price_type);
					...
					if($arOrderChildren["PERSON_TYPE_ID"]==2) // если юрлицо
					{
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 28, "NAME" => "Группа товаров в заказе", "CODE" => "ORDER_CATEGORY", "VALUE" => $group_name);
						CSaleOrderPropsValue::Add($arFields);
						$arFields = array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => 29, "NAME" => "Типы цен в заказе", "CODE" => "PRICE_TYPE", "VALUE" => $price_type);
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if(isset($ORDER_CHILDREN))
					{
						$arResult["ORDER_ID"] = $ORDER_WOMEN;
						$totalOrderPrice = $orderWomenSum;
					}
					...
					if(isset($ORDER_CHILDREN)) 
					{
						...
						$arFields = Array(
							...
							"ORDER_DATE" => Date($DB->DateFormatToPHP(CLang::GetDateFormat("SHORT", SITE_ID))),
							"ORDER_USER" => ( (strlen($arUserResult["PAYER_NAME"]) > 0) ? $arUserResult["PAYER_NAME"] : $USER->GetFormattedName(false)),
							"PRICE" => SaleFormatCurrency($orderChildrenSum, $arResult["BASE_LANG_CURRENCY"]),
							"BCC" => COption::GetOptionString("sale", "order_email", "order@".$SERVER_NAME),
							"EMAIL" => (strlen($arUserResult["USER_EMAIL"])>0 ? $arUserResult["USER_EMAIL"] : $USER->GetEmail()),
							"ORDER_LIST" => $strOrderList,
							"SALE_EMAIL" => COption::GetOptionString("sale", "order_email", "order@".$SERVER_NAME),
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if(isset($ORDER_CHILDREN)) 
						$arResult["REDIRECT_URL"] = $APPLICATION->GetCurPageParam("ORDER_ID=".urlencode(urlencode($arOrder["ACCOUNT_NUMBER"])).",".$ORDER_CHILDREN, Array("ORDER_ID"));
				}
				else
	...
	else
	{
		...
		$t = explode(",", $_REQUEST["ORDER_ID"]);
		$ID = intVal($t[0]);
		if(isset($t[1])) $ID_2 = intVal($t[1]);
			...
			if ($arOrder && $arOrder["USER_ID"] == IntVal($USER->GetID()))
			{
				if (IntVal($arOrder["PAY_SYSTEM_ID"]) > 0 && $arOrder["PAYED"] != "Y")
				{
					...
					if ($arPaySysAction = $dbPaySysAction->Fetch())
					{
						...
						$arResult["PAY_SYSTEM_2"] = $arPaySysAction;
					}
				}
				$arResult["ORDER_2"] = $arOrder;	
