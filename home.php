<?
//die("newvay.ru");
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
$APPLICATION->SetTitle("Трикотаж оптом от производителя. Женский и детский трикотаж российского производства - Фемина-Трейд");
$APPLICATION->SetPageProperty("keywords", "Трикотаж российского производства оптом от Кимрской трикотажной фабрики. Детский и женский трикотаж оптом от производителя на заказ");
$APPLICATION->SetPageProperty("description", "Трикотаж российского производства оптом от Кимрской трикотажной фабрики. Детский и женский трикотаж оптом от производителя на заказ");
?>

	
	<?$APPLICATION->IncludeComponent(
	"bitrix:news.list", 
	"front-slider", 
	array(
		"ACTIVE_DATE_FORMAT" => "d.m.Y",
		"ADD_SECTIONS_CHAIN" => "N",
		"AJAX_MODE" => "N",
		"AJAX_OPTION_ADDITIONAL" => "",
		"AJAX_OPTION_HISTORY" => "N",
		"AJAX_OPTION_JUMP" => "N",
		"AJAX_OPTION_STYLE" => "Y",
		"CACHE_FILTER" => "N",
		"CACHE_GROUPS" => "Y",
		"CACHE_TIME" => "36000000",
		"CACHE_TYPE" => "N",
		"CHECK_DATES" => "Y",
		"DETAIL_URL" => "",
		"DISPLAY_BOTTOM_PAGER" => "N",
		"DISPLAY_DATE" => "N",
		"DISPLAY_NAME" => "N",
		"DISPLAY_PICTURE" => "Y",
		"DISPLAY_PREVIEW_TEXT" => "N",
		"DISPLAY_TOP_PAGER" => "N",
		"FIELD_CODE" => array(
			0 => "DETAIL_PICTURE",
			1 => "",
		),
		"FILTER_NAME" => "",
		"HIDE_LINK_WHEN_NO_DETAIL" => "N",
		"IBLOCK_ID" => "16",
		"IBLOCK_TYPE" => "news",
		"INCLUDE_IBLOCK_INTO_CHAIN" => "N",
		"INCLUDE_SUBSECTIONS" => "N",
		"MESSAGE_404" => "",
		"NEWS_COUNT" => "20",
		"PAGER_BASE_LINK_ENABLE" => "N",
		"PAGER_DESC_NUMBERING" => "N",
		"PAGER_DESC_NUMBERING_CACHE_TIME" => "36000",
		"PAGER_SHOW_ALL" => "N",
		"PAGER_SHOW_ALWAYS" => "N",
		"PAGER_TEMPLATE" => ".default",
		"PAGER_TITLE" => "Новости",
		"PARENT_SECTION" => "",
		"PARENT_SECTION_CODE" => "",
		"PREVIEW_TRUNCATE_LEN" => "",
		"PROPERTY_CODE" => array(
			0 => "LINK",
			1 => "",
		),
		"SET_BROWSER_TITLE" => "N",
		"SET_LAST_MODIFIED" => "N",
		"SET_META_DESCRIPTION" => "N",
		"SET_META_KEYWORDS" => "N",
		"SET_STATUS_404" => "N",
		"SET_TITLE" => "N",
		"SHOW_404" => "N",
		"SORT_BY1" => "ACTIVE_FROM",
		"SORT_BY2" => "SORT",
		"SORT_ORDER1" => "DESC",
		"SORT_ORDER2" => "ASC",
		"COMPONENT_TEMPLATE" => ".default"
	),
	false
);
	?>
	
	
	
	<?$APPLICATION->IncludeComponent("bitrix:catalog.section.list", "catalog-root-home", Array(
	"IBLOCK_TYPE" => "1c_catalog",	// Тип инфоблока
		"IBLOCK_ID" => "6",	// Инфоблок
		"SECTION_ID" => "",	// ID раздела
		"SECTION_CODE" => "",	// Код раздела
		"COUNT_ELEMENTS" => "Y",	// Показывать количество элементов в разделе
		"TOP_DEPTH" => "2",	// Максимальная отображаемая глубина разделов
		"SECTION_FIELDS" => array(	// Поля разделов
			0 => "",
			1 => "",
		),
		"SECTION_USER_FIELDS" => array(	// Свойства разделов
			0 => "UF_COLLECTION_COLOR",
			1 => "",
		),
		"SECTION_URL" => "/#SECTION_CODE_PATH#/",	// URL, ведущий на страницу с содержимым раздела
		"CACHE_TYPE" => "N",	// Тип кеширования
		"CACHE_TIME" => "3600",	// Время кеширования (сек.)
		"CACHE_GROUPS" => "N",	// Учитывать права доступа
		"ADD_SECTIONS_CHAIN" => "N",	// Включать раздел в цепочку навигации
		"COMPONENT_TEMPLATE" => "catalog-root"
	),
	false
);?>




<h1>Трикотаж оптом от производителя в Москве</h1>
<p>Компания «Фемина Трейд» — российский производитель вязаных и швейных изделий для детей и взрослых.</p>
<p>Основа компании — фабрика «Меланж», созданная на базе одного из крупнейших советских предприятий легкой промышленности «Кимрский льнотрикотаж».</p>
<p>Мы сотрудничаем с юридическими (ООО, ИП) и физлицами:</p>
<ul>
<li>торговыми сетями;</li>
<li>владельцами магазинов и точек в ТЦ;</li>
<li>розничными интернет-магазинами;</li>
</ul>
<p>Приобретая российский трикотаж оптом от производителя напрямую, без посредников, вы:</p>
<ul>
<li>уменьшаете себестоимость товара и увеличиваете прибыль;</li>
<li>ускоряете формирование и пополнение ассортимента вашего магазина;</li>
<li>не зависите от курса валют, таможенных пошлин и санкций.</li>
</ul>
<p>Географических ограничений нет — заказать трикотаж оптом в Москве можно из любой точки РФ.</p>
<p>Минимальная сумма заказа — 15 000 рублей.</p>

<h2>Российский трикотаж оптом: наш ассортимент</h2>
<p>Предлагая российский трикотаж оптом, мы составляем каталог с учетом разноплановых интересов конечного потребителя. Каждый сезон выпускаем более 250 вязаных моделей, дополняем их швейными изделиями и аксессуарами. Вязаные шапки, шарфы, снуды, варежки, митенки, гетры — отличные дополнения к одежному ассортименту вашего магазина.</p>
<p>В продаже: летняя одежда (топы, сарафаны, футболки, шорты) и демисезонная (свитера, джемперы, блузы, пуловеры, платья, жилеты, брюки, болеро и пр.). Также можно приобрести верхний трикотаж оптом (вязаные пальто, кардиганы, теплые жакеты).</p>
<p>Для производства используется только высококаче­ственная пряжа и полотно с большим содержанием натуральных волокон (шерсть, хлопок, вискоза).</p>
<p>Наши торговые марки</p>
<p><strong>VAY</strong> — повседневные модели в стиле casual женские и мужские: вязаные жилеты, жакеты, кардиганы, джем­перы, платья, костюмы. А также швейные изделия из трикотажного полотна: топы, блузы, футболки, юбки, платья и сарафаны. Ряды размеров — 42–62.</p>
<p><strong>VAY-KIDS</strong> — вязаный трикотаж для детей 7–14 лет:</p>
<ul>
<li>школьная форма — платья, сарафаны, джемперы, жилеты, кардиганы, джемперы из хлопковой или полушерстяной пряжи (30–50 размеры);</li>
<li>повседневные модели 30–44 размеров.</li>
</ul>
<p><strong>«Веснушки»</strong> — вязаная одежда для девочек и мальчиков 2–7 лет (рост от 92 до 122 см).</p>
<p>Новинка среди предложений компании «Фемина Трейд» — линейка одежды для всей семьи <strong>FAMILY LOOK</strong>, модели одинакового дизайна.</p>

<h2>Почему стоит заказывать трикотаж именно у нас?</h2>
<ol>
	<li>Вы можете выбрать удобную форму заказа:
		<ul>
			<li>со свободного склада — постоянно обновляющийся широкий ассортимент (более 500 моделей), актуальное наличие товара на сайте.</li>
			<li>предзаказ — товар поступит к вам точно в означенный срок по фиксированной цене;</li>
			<li>индивидуальный заказ на выпуск продукции под вашей торговой маркой с гарантией эксклюзивного права на продажу.</li>
		</ul>
	</li>
	<li>Заказать трикотаж можно не размерными рядами, а выборочно.</li>
	<li>Качество трикотажа соответствует строгим отечественным стандартам, что подтверждено сертификатами соответствия, а также экспертами АНО «Союзэкспертиза».
		<p>В 2015 году «Фемина Трейд» стала победителем среди российских и иностранных компаний в номинации «За высокие потребительские свойства товаров».</p>
		<p>Компания отмечена многочисленными наградами профильных выставок и награждена почетным знаком «За большой вклад в формирование национального рынка товаров отечественного производства».</p>
	</li>
	<li>Интернет-магазин — собственный торговый дом фабрики, что обеспечивает вам возможности:
		<ul>
			<li>закупать дешевый трикотаж оптом — цены без посреднических надбавок, не зависят от курса валют;</li>
			<li>пользоваться скидками в рамках акций и по программам лояльности;</li>
			<li>оперативно оформлять заказы онлайн.</li>
		</ul>
	</li>
	<li>Подразделения компании (демонстрационный зал, офис, склад, автопарк) расположены на общей территории.</li>
	<li>Резервирование товара на 5 дней с момента выставления счета.</li>
	<li>Доставка или самовывоз — на ваш выбор.</li>
	<li>Отгрузка не позднее чем на 3-й рабочий день после оплаты на р/с либо в кассу.</li>
	<li>Пакет документов заказчика минимален.</li>
</ol>

<h2>Хотите купить трикотаж оптом — действуйте!</h2>
<p>Купить трикотаж оптом в Москве в компании «Фемина Трейд» — значит поддержать отечественного производителя и обеспечить выгодное развитие собственному бизнесу.</p>
<br>	
	
	<?/*?>
    <section class="font-slider">
        <div class="slide"><img src="/bitrix/templates/new_femina/images/slide-1.jpg"></div>
        <div class="slide"><img src="/bitrix/templates/new_femina/images/slide-2.jpg"></div>
        <div class="slide"><img src="/bitrix/templates/new_femina/images/slide-3.jpg"></div>
        <div class="slide"><img src="/bitrix/templates/new_femina/images/slide-4.jpg"></div>
        <div class="slide"><img src="/bitrix/templates/new_femina/images/slide-5.jpg"></div>
        <div class="slide"><img src="/bitrix/templates/new_femina/images/slide-6.jpg"></div>
    </section>
    <section class="content">
        <div class="items catalog-link">
            <div class="item items-2 vay"><a href="/vay/"><img src="/bitrix/templates/new_femina/images/vay_front.jpg" alt="Женский трикодаж"></a></div>
            <div class="item items-2"><a href="/detskiy-trikotazh/"><img src="/bitrix/templates/new_femina/images/vay_kids_front.jpg" alt="Детский трикотаж"></a></div>
        </div>
        <a class="items last-new" href="/muzhskoy-trikotazh/"></a>
        <a href="/stati/" class="items articles">
            <img src="/bitrix/templates/new_femina/images/articles_front.jpg" alt="Статьи">
            <div class="article-wrap">
                <h2>СТАТЬИ</h2>
                <div class="deck">УНИКАЛЬНЫЕ СВОЙСТВА ТРИКОТАЖА ИЗ БАМБУКОВОГО ВОЛОКНА</div>
            </div>
        </a>
        <a href="/o-kompanii/o-proizvodstve.html" title="О ПРОИЗВОДСТВЕ" class="items about"><span>О ПРОИЗВОДСТВЕ</span></a>
    </section>
	<?*/?>
<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/footer.php");?>