...
if(IntVal($ORDER_ID) <= 0)
{
	...
	*/
	...
	/*
	 * register user if to order busket
	 */
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["BasketOrder"]) AND !$USER->IsAuthorized())
	{
		...
		*/ 
		...
		//if (count($arUser) > 1)
		if ((count($arUser) > 1) && (strLen($payerEMail)>0) ) 										// inmark fix
		...
		if (strlen($errorMessage) <= 0)
		{
			...
			if ($user_id > 0 AND count($arErrors) <= 0)
			{
	...
	*/
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["BasketOrder"]) AND strlen($errorMessage) <= 0 AND check_bitrix_sessid())
	{
		...
		if (strlen($errorMessage) <= 0)
		{
			...
			if ($ORDER_ID > 0 AND count($arErrors) <= 0)
			{
	...
	if (is_array($arBasketItems["BASKET_ITEMS"]))
	{
		...
		*/
		if (isset($display_props) AND $display_props == "block")
			$arParams['SHOW_BUSKET_ORDER'] = "Y";

		if (isset($display_props) AND $display_props == "none")
			$arParams['SHOW_BUSKET_ORDER'] = "N";
...
{
	$arResult["ORDER_BUSKET"]["CONFIRM_ORDER"] = "Y";
	$arResult["ORDER_BUSKET"]["ORDER_ID"] = IntVal($ORDER_ID);
	...
	$dbOrder = CSaleOrder::GetList(
		...
		array(
				"LID" => SITE_ID,
				"USER_ID" => IntVal($USER->GetID()),
				"ID" => $arResult["ORDER_BUSKET"]["ORDER_ID"]
		)
	);
