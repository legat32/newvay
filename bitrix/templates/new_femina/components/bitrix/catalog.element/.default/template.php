...
{
?>
	<div class="bx_stick average left bottom" id="<? echo $arItemIDs['STICKER_ID'] ?>" title="<? echo $arResult['LABEL_VALUE']; ?>"><? echo $arResult['LABEL_VALUE']; ?></div>
<?
}
?>
	</div>
...
<?
	...
	else
	{
		foreach ($arResult['OFFERS'] as $key => $arOneOffer)
		{
			...
			if($arOneOffer['MORE_PHOTO_COUNT'] == 1) $arResult['OFFERS'][$key]['MORE_PHOTO_COUNT'] = 0;
			//prn($arResult['OFFERS'][$key]['MORE_PHOTO_COUNT']);
...
<div class="bitrix_cart<?if(!is_array($minPrice)):?> not-buy<?endif?>">
	
	<?
	$boolDiscountShow = (0 < $minPrice['DISCOUNT_DIFF']);
	$pricetype = $arResult["CATALOG_GROUP_NAME_".$arResult["PRICES_ALLOW"][0]] ? $arResult["CATALOG_GROUP_NAME_".$arResult["PRICES_ALLOW"][0]] : "";
	?>
	<div class="item_price"<?=!$boolDiscountShow ? ' style="height: 70px;"' : ''?>>
		<div class="label-price"><?=$pricetype?></div>
		<?if($boolDiscountShow):?>
	...
	<div class="offers">
		...
		?>	
			...
			<a rel="nofollow" class="fancybox-ajax" id="table_nal" href="/ajax/nalichie.php?ID=<?=$arResult["ID"]?>">Таблица наличия</a>
...
<?if(!is_array($minPrice)):?>
	<div class="bitrix_cart">
		<div class="item_price">Для просмотра оптовой цены Вам необходимо зарегистрироваться</div>
...
<?
			if($arProp['ID']==193){
				echo '<span id="color_title">Выберите цвет:</span>';
			}
			elseif ($arProp['ID']==194) {
				echo '<span id="size_title">Выберите размер:</span>';
			}
		?>
		<div class="color_vars_<? echo $arProp['ID']; ?> wrap">
		
			<div class="frame smart" id="smart">
				<ul id="<? echo $arItemIDs['PROP'].$arProp['ID']; ?>_list">
				<?foreach ($arProp['VALUES'] as $arOneValue){
					$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);?>
					<li class="color_var" data-treevalue="<? echo $arProp['ID'].'_'.$arOneValue['ID']; ?>" data-onevalue="<? echo $arOneValue['ID']; ?>">
						<?if($arProp['ID'] == 193):?>
							<span class="fa fa-camera<?if(!isset($arResult["EX_COLORS_FOTO"][trim(strtoupper($arOneValue['NAME']))])):?> no-ex<?endif?>"></span>
						<?endif?>
...
<div class="color_vars wrap" id="<? echo $arItemIDs['PROP'].$arProp['ID']; ?>_cont">

			<ul id="<? echo $arItemIDs['PROP'].$arProp['ID']; ?>_list">
			<?foreach ($arProp['VALUES'] as $arOneValue){
				$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);?>
				<li data-treevalue="<? echo $arProp['ID'].'_'.$arOneValue['ID'] ?>" data-onevalue="<? echo $arOneValue['ID']; ?>">
					<i title="<? echo $arOneValue['NAME']; ?>"></i>
					<span class="cnt"><span class="cnt_item" title="<? echo $arOneValue['NAME']; ?>"></span></span>
				</li>
			<?}?>
			</ul>
		<div class="bx_slide_left" style="display: none;" id="<? echo $arItemIDs['PROP'].$arProp['ID']; ?>_left" data-treevalue="<? echo $arProp['ID']; ?>"></div>
