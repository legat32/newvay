...
//$arResult["PROPERTIES"]["PHOTO_STORAGE"]["VALUE"] = "OFFERS";
//pra($arResult["PROPERTIES"]["PHOTO_STORAGE"]["VALUE"]);
...
if(function_exists("sortOffersInItem"))
	$arResult = sortOffersInItem($arResult);
...
$strEmptyPreview = '/upload/no_female_big.gif';
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		// комментим, ибо фото в торговые предложения расставляются по собственному алгоритму
		...
		if($arResult["PROPERTIES"]["PHOTO_STORAGE"]["VALUE"] == "OFFERS")
		{
			// комментим, ибо фото в торговые предложения расставляются по собственному алгоритму
			$arOffer['MORE_PHOTO'] = array();
			$arOffer['MORE_PHOTO_COUNT'] = 0;
	...
	// комментим, ибо сверху задана своя сортировка
...
//$arResult["NO_MAIN_FOTO"] = "Y";       // установим флаг остутствия главного фото
...
prn($arResult['OFFERS_PROP']);
/*
foreach($arResult["OFFERS"] as $arOffer) 
{
	prn($arOffer["NAME"]);
	prn($arOffer["PREVIEW_PICTURE"]);
...
foreach($arResult["OFFERS"] as $arOffer)
{
	...
	//pra($arParams['OFFER_ADD_PICT_PROP']);
	//pra($arParams['ADD_DETAIL_TO_SLIDER']);
	//$offerSlider = CIBlockPriceTools::getSliderForItem($arOffer, $arParams['OFFER_ADD_PICT_PROP'], $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y');
	//pra($offerSlider);
	//pra($arOffer["PROPERTIES"]["MORE_PHOTO"]);
	//pra($arOffer["MORE_PHOTO"]);
	//pra($arOffer["MORE_PHOTO_COUNT"]); 
}
