						...
						if($value["#"][GetMessage("CC_BSC1_OPERATION")][0]["#"] == GetMessage("CC_BSC1_ORDER"))
						{
							...
							if($orderInfo = CSaleOrder::GetByID($orderId))
							{
								...
								else
								{
									...
									// ---- Set Status "M" if order came from 1c in first time ( Naumov Inmarketing )
									if ((strlen($v["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")])<1) && (strlen($v["TRAITS"][GetMessage("CC_BSC1_1C_DELIVERY_DATE")])<1) ) {
										CSaleOrder::StatusOrder($orderId, "M");
										}
									// ---------------------------
									...
									if(strlen($v["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")])>1)
									{
										if($orderInfo["PAYED"]=="N") {
											...
											// ---- Set Status "P" if order payed in first time ( Naumov Inmarketing )
