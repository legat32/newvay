...
$arOfferIDs = Array();
foreach($arResult["GRID"]["ROWS"] as $id => $arProduct)
{
	$arOfferIDs[] = $arProduct["data"]["PRODUCT_ID"];
}
//pra($arOfferIDs);


if(count($arOfferIDs) > 0)
{
	$arImages = Array();
	...
	foreach($arResult["GRID"]["ROWS"] as $itemID => $arItem)
	{
		...
		{
			if(strpos(" ".$arPicture["NAME"], $arItem["data"]["PROPS"][0]["VALUE"]) > 0)
			{
				...
				$arResult["GRID"]["ROWS"][$itemID]["data"]["PREVIEW_PICTURE"] = $arFile;
				break;
			}
		}
					...
					?>
						<td class="itemphoto">
							<div class="bx_ordercart_photo_container">
								<?//pra($arData["data"]["PREVIEW_PICTURE"])?>
								<?
								/*
