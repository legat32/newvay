...
<?if(!defined("PRICE_TYPE")):?>
	<?$APPLICATION->SetAdditionalCSS("/bitrix/templates/femina_light/components/bitrix/catalog.smart.filter/visual_horizontal_new/style_for_prices.css");?>
<?endif?>
			...
			foreach($arResult["ITEMS"] as $key=>$arItem)
			{
				...
				if(isset($arItem["PRICE"])):
					if (!$arItem["VALUES"]["MIN"]["VALUE"] || !$arItem["VALUES"]["MAX"]["VALUE"] || $arItem["VALUES"]["MIN"]["VALUE"] == $arItem["VALUES"]["MAX"]["VALUE"])
						continue;
					?>
					<div id="prop_<?=$arItem["ID"]?>" class="bx_filter_parameters_box active">
					...
					<div class="bx_filter_block">
						...
						switch ($arItem["DISPLAY_TYPE"])
						{
							...
							default://CHECKBOXES
								...
								<?if($arItem["ID"] != 60):  // размеры отобразим по-своему ?>
									<?foreach($arItem["VALUES"] as $val => $ar):?>
								...
								<?if($arItem["ID"] == 60):  // размеры отобразим по-своему ?>
									...
									<?
									$columns_count = 7;
									$column_count = ceil(count($arItem["VALUES"])/$columns_count);
									$num = 0;
									?>
									...
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										...
										<?if($num % $column_count == ($column_count-1)):?>
											</div>
										<?endif?>
										<?$num++?>
									<?endforeach;?>
			...
			<div class="bx_filter_button_box active">
				<div class="bx_filter_block">
					<div class="bx_filter_parameters_box_container">
						...
						<div class="bx_filter_popup_result left" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"'; else echo 'style="display: inline-block;"'?>>
