...
<div class="bx_my_order_switch">
	<a class="bx_mo_link" href="/personal/orders/"><?=GetMessage('SPOD_CUR_ORDERS')?></a>
</div>
...
<div class="bx_order_list">
	...
	<?else:?>
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						$arUser = $dbUser->Fetch();
						$partner = intVal(trim($arUser["UF_PARTNER_TYPE"]));
						$rsUserField = CUserFieldEnum::GetList(array(), array("ID" => $partner));
					...
					<tr>
						...
						<td class="custom name">
							...
							<?foreach($prod["PROPS"] as $prop):?>
								<?=str_replace("Справочник ", "", $prop["NAME"])?>: <?=$prop["VALUE"]?><br>
							<?endforeach?>
							</span>
						</td>
						...
						<td class="custom price">
							...
							<?if(doubleval($prod["DISCOUNT_PRICE"]) > 0):?>
								<div class="old_price">
									<s><?=SaleFormatCurrency($prod["PRICE"] + $prod["DISCOUNT_PRICE"], $prod["CURRENCY"])?></s>
								</div>
							<?endif?>	
							<span class="price-type"><?=$prod["NOTES"]?></span>
						</td>

						<?if($arResult['HAS_DISCOUNT']):?>
							<td class="custom price"><?=$prod["DISCOUNT_PRICE_PERCENT_FORMATED"]?></td>
						<?endif?>

						<td class="custom amont"><?=$prod["QUANTITY"]?> шт</td>
		...
		<table class="bx_ordercart_order_sum" style="width: 100%">
			<tbody>

				<? ///// WEIGHT ?>
				<?if(floatval($arResult["ORDER_WEIGHT"])):?>
					<tr>
						<td class="custom_t1" style="width:80%; text-align:right;"><?=GetMessage('SPOD_TOTAL_WEIGHT')?>:</td>
						<td class="custom_t2" style="width:20%; text-align:right;"><?=$arResult['ORDER_WEIGHT_FORMATED']?></td>
					</tr>
				<?endif?>
				...
				<?/*?>
				<tr>
					<td class="custom_t1" style="width:80%; text-align:right;"><?=GetMessage('SPOD_PRODUCT_SUM')?>:</td>
					<td class="custom_t2" style="width:20%; text-align:right;"><?=$arResult['PRODUCT_SUM_FORMATTED']?></td>
				</tr>
				<?*/?>
				...
				<?if(strlen($arResult["PRICE_DELIVERY_FORMATED"])):?>
					<tr>
						<td class="custom_t1" style="width:80%; text-align:right;"><?=GetMessage('SPOD_DELIVERY')?>:</td>
						<td class="custom_t2" style="width:20%; text-align:right;"><?=$arResult["PRICE_DELIVERY_FORMATED"]?></td>
					</tr>
				<?endif?>
				...
				<tr>
					<td class="custom_t1 fwb" style="width:80%; text-align:right;"><?=GetMessage('SPOD_SUMMARY')?>:</td>
					<td class="custom_t2 fwb" style="width:20%; text-align:right;"><?=$arResult["PRICE_FORMATED"]?></td>
				</tr>
			</tbody>
		</table>
		<br>
