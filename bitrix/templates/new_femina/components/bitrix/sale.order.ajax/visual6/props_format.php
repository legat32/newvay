	...
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default")
	{
		...
		if (!empty($arSource))
		{
			?>
					...
					{
						if($group_name != $arProperties["GROUP_NAME"])
						{
							$group_name = $arProperties["GROUP_NAME"];
							?><h4><?=str_replace("(юр.)", "", $arProperties["GROUP_NAME"])?></h4><?
						}
						...
						if ($arProperties["TYPE"] == "CHECKBOX")
						{
							...
							<div class="property <?=$arProperties["FIELD_NAME"]?>">
								<label>
									<span class="property-name"><?=$arProperties["NAME"]?>
							...
							<div class="property <?=$arProperties["FIELD_NAME"]?>">
								...
								<select name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" size="<?=$arProperties["SIZE1"]?>">
									...
									foreach($arProperties["VARIANTS"] as &$arVariants):
										...
										// если значение равно empty то делаем, что это поле не выбрано
										
										if( ($arVariants["VALUE"] == "empty") && ($arVariants["SELECTED"] == "Y") ) 
										{
											unset($arVariants["SELECTED"]);
											$arVariants["VALUE"] = "";
										}
									?>
						...
						elseif ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							<div class="property <?=$arProperties["FIELD_NAME"]?> <?=$arProperties["TYPE"]?>">
								<label>
									<span class="property-name"><?=$arProperties["NAME"]?>
								<?if ($arProperties["REQUIED_FORMATED"]=="Y"):?>
									<span class="bx_sof_req">*</span>
