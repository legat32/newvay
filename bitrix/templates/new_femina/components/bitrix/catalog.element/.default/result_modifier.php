...
// ===========================================================================================
//   Для подсветки цветов с фото составим список цветов, в которых есть фото
// ===========================================================================================
$arColors = Array();
foreach($arResult["OFFERS"] as $arOffer) 
{
	unset($tmp_color);
	$tmp_color = strtoupper(trim($arOffer["PROPERTIES"]["COLOR_LIST"]["VALUE"])); 
	if(is_array($arOffer["MORE_PHOTO"]) && ($arOffer["MORE_PHOTO"][0]["ID"] != NOFOTO_FILE_ID) && !isset($arColors[$tm_color]) )
		$arColors[$tmp_color] = 1; 
	//pra($arOffer["NAME"]);
}
$arResult["EX_COLORS_FOTO"] = $arColors;
...
if($arResult["PROPERTIES"]["NOVIZNA"]["VALUE"] == "Основной каталог") unset($arResult["LABEL"]);
