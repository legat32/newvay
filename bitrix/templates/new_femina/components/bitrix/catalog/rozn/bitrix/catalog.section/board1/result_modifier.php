...
if (!empty($arResult['ITEMS']))
{
	...
	foreach($arResult["ITEMS"] as $arItem) 
	{
		echo "<h1>[".$arItem["ID"]."] ".$arItem["NAME"]."<h1>";
		prn($arItem["DETAIL_PICTURE"]);
		$t = explode("ЦВЕТ", strtoupper($arItem["DETAIL_PICTURE"]["DESCRIPTION"]));
		$FIRST_COLOR = strtoupper(trim($t[1]));
		prn($FIRST_COLOR);
		foreach($arItem["OFFERS"] as $arOffer)
		{
			prn("[".$arOffer["ID"]."] ".$arOffer["NAME"]." - ".$arOffer["DETAIL_PICTURE"]["DESCRIPTION"]);
		}
	}
	*/
	...
	foreach($arResult["ITEMS"] as $arItem)
	{
		//pra($arItem["NAME"]."<br>".$arItem["PROPERTIES"]["SEZON"]["VALUE"]."<br>".$arItem["PROPERTIES"]["NOVIZNA"]["VALUE"]."<br>".$arItem["PROPERTIES"]["NOVINKA"]["VALUE"]);
	}
